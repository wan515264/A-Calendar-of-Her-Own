...
Zany（疯癫）对应生产。它是一种关于表演、忙碌、多工、情感劳动以及持续适应的审美。疯癫的人始终在工作、娱乐、协调与表演。`
  ...
  },
  {
    ...
    summaryEn:
      "For months, Annie Ernaux’s narrator lives almost entirely in anticipation of a man’s arrival. Everything else—work, television, shopping, conversation—becomes background noise. Time no longer moves forward normally. It stretches, pauses, and begins again with every phone call, every visit, every absence.",

    summaryZh:
      "几个月里，安妮·埃尔诺笔下的叙述者几乎完全生活在对一个男人到来的期待中。工作、电视、购物、交谈，都逐渐退到背景。时间不再正常向前流动，而是在每一通电话、每一次相见、每一次缺席之间被拉长、暂停，又重新开始。",

    highlightEn:
      "Passion does not only change whom we love. It changes how we experience time itself.",

    highlightZh:
      "激情改变的不只是我们爱谁。它甚至改变了我们经历时间的方式。",
