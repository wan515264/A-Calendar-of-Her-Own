...
Zany（疯癫）对应生产。它是一种关于表演、忙碌、多工、情感劳动以及持续适应的审美。疯癫的人始终在工作、娱乐、协调与表演。`
  ...
  },
  {
    ...
    introEn: `Simple Passion begins with an experience that seems private and almost impossible to explain: one woman’s complete absorption in an affair.

The man is absent for much of the book. He appears, disappears, calls, visits, and leaves. Yet his physical absence does not make him less present. On the contrary, he comes to organise the narrator’s entire inner world.
...
埃尔诺并不把它写成理性的失败。她只是表明：它之所以真实，是因为它确实被经历过。`
    ...
    researchLens: {
      ...
      titleZh: "等待、时间与身体",
      citation: "An editorial reading of Annie Ernaux, Simple Passion.",
      summaryEn: `Passion does not only change whom we love. It changes how we experience time itself.

In Simple Passion, waiting becomes a structure of life. The narrator no longer experiences time as neutral sequence, but as distance from a phone call, a visit, a body, or an absence.

The book’s force comes from its refusal of self-defence. Ernaux does not moralize desire or decorate it with dignity. She writes the altered reality of passion as precisely as possible.`,
      summaryZh: `激情改变的不只是我们爱谁。它甚至改变了我们经历时间的方式。

在《简单的激情》中，等待成为生活的结构。叙述者不再把时间经验为中性的顺序，而是把它经验为与一通电话、一次来访、一个身体或一次缺席之间的距离。

这本书的力量来自它拒绝为自己辩护。埃尔诺没有把欲望道德化，也没有用尊严来装饰它。她只是尽可能准确地写出激情所造成的现实变形。`
    },
    quote: "I measured time differently, with all my body.",
